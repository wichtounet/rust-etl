...
pub trait EtlExpr<T: EtlValueType>: std::marker::Sync + Clone {
        ...
        if RightExpr::THREAD_SAFE && size > PARALLEL_THRESHOLD {
            rayon::scope(|s| {
                ...
                for t in 0..n {
                    ...
                    s.spawn(move |_| {
                        for (x, v) in slice.iter_mut().zip(rhs.iter_range(start..end)) {
                            *x = v;
                        }
                    });
                }
...
pub fn axpy_direct<T: EtlValueType, RightExpr: EtlExpr<T>>(x: &mut Vec<T>, alpha: T, beta: T, y: &RightExpr) {
    let size = if RightExpr::DIMENSIONS == 0 {
        x.len()
    } else if RightExpr::TYPE != EtlType::Unaligned {
        padded_size(y.size())
    ...
    if RightExpr::THREAD_SAFE && size > PARALLEL_THRESHOLD {
        rayon::scope(|s| {
            ...
            for t in 0..n {
                ...
                s.spawn(move |_| {
                    for (x, v) in slice.iter_mut().zip(y.iter_range(start..end)) {
                        *x = alpha * *x + beta * v;
                    }
                });
            }
        ...
        for (lhs, rhs) in x[0..size].iter_mut().zip(y.iter()) {
            *lhs = alpha * *lhs + beta * rhs;
        }
    }
}
