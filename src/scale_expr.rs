...
pub struct ScaleExprIterator<'a, T: EtlValueType, LeftExpr: EtlExpr<T> + 'a, RightExpr: EtlExpr<T> + 'a>
where
    T: 'a,
{
...
impl<'a, T: EtlValueType, LeftExpr: EtlExpr<T>, RightExpr: EtlExpr<T>> Iterator for ScaleExprIterator<'a, T, LeftExpr, RightExpr> {
    type Item = T;
...
impl<T: EtlValueType, LeftExpr: WrappableExpr<T>, RightExpr: WrappableExpr<T>> EtlExpr<T> for ScaleExpr<T, LeftExpr, RightExpr> {
    ...
    type Iter<'x>
        = ScaleExprIterator<'x, T, LeftExpr::WrappedAs, RightExpr::WrappedAs>
    where
        T: 'x,
        Self: 'x;
...
#[macro_export]
macro_rules! impl_scale_op_constant_trait {
    ($trait:tt, $type:ty) => {
        impl<T: EtlValueType + $trait, RightExpr: WrappableExpr<T>> std::ops::Shr<RightExpr> for $type {
            type Output = $crate::scale_expr::ScaleExpr<T, $type, RightExpr>;
...
#[macro_export]
macro_rules! impl_scale_op_binary_expr_simd {
    ($type:ty) => {
        impl<T: EtlValueType, LeftExpr: WrappableExpr<T>, RightExpr: WrappableExpr<T>, OuterRightExpr: WrappableExpr<T>> std::ops::Shr<OuterRightExpr> for $type
        where
            Simd<T, 8>: SimdHelper,
        {
            type Output = $crate::scale_expr::ScaleExpr<T, $type, OuterRightExpr>;
